...
(defun make_message (&optional  port  datum)                                                                            #|line 185|#
    (let ((p (clone_string port                                                                                         #|line 186|#)))
        (let ((m (funcall 'Message  :port  p :datum (cdr (assoc '(clone )  datum))                                      #|line 187|#)))
...
(defun message_clone (&optional  message)                                                                               #|line 192|#
    (let ((m (funcall 'Message  :port (funcall 'clone_port   (cdr (assoc ' port  message)) ) :datum (cdr (assoc '(cdr (assoc '(clone )  datum))  message)) #|line 193|#)))
...
(defun log_inject (&optional  receiver  port  msg)                                                                      #|line 315|#
    (let ((inject_desc (make_Inject_Descriptor :receiver  receiver :port  port :message  msg                            #|line 316|#)))
        (append_routing_descriptor :container  receiver :desc  inject_desc                                              #|line 317|#))#|line 318|#
)
...
(defun log_inout_no_output (&optional  container  component  in_message)                                                #|line 485|#
    (let ((rdesc (make_InOut_Descriptor :container  container :component  component :in_message  in_message             #|line 486|#:out_port  nil :out_message  nil #|line 487|#)))
        (append_routing_descriptor container   rdesc                                                                    #|line 488|#))#|line 489|#
)
(defun log_inout_single (&optional  container  component  in_message  out_message)                                      #|line 491|#
    (let ((rdesc (make_InOut_Descriptor :container  container :component  component :in_message  in_message             #|line 492|#:out_port  nil :out_message  out_message #|line 493|#)))
        (append_routing_descriptor container   rdesc                                                                    #|line 494|#))#|line 495|#
)
...
(defun fmt_inout (&optional  desc  indent)                                                                              #|line 507|#
    (let ((outm (cdr (assoc 'out_message  desc))))                                                                      #|line 508|#
        (cond
          ...
          (t                                                                                                            #|line 511|#
                (return-from fmt_inout  (concatenate 'string  "\n"  (concatenate 'string  indent  (concatenate 'string  "  ∴ "  (concatenate 'string (cdr (assoc ' name (cdr (assoc 'component  desc))))  (concatenate 'string  " " (format_message outm )))))))#|line 516|##|line 517|#
            )))                                                                                                         #|line 518|#
)
...
(defun routing_trace_all (&optional  container)                                                                         #|line 525|#
    (let ((indent  ""))                                                                                                 #|line 526|#
        (let ((lis (funcall 'list   (cdr (assoc '(cdr (assoc ' queue  routings))  container))                           #|line 527|#)))
            (return-from routing_trace_all (recursive_routing_trace container   lis   indent                            #|line 528|#))))#|line 529|#
)
      ...
      (t                                                                                                                #|line 534|#
            (let ((desc (first lis                                                                                      #|line 535|#)))
                (let ((formatted (funcall (cdr (assoc 'fmt  desc))    desc   indent                                     #|line 536|#)))
                    (return-from recursive_routing_trace (+  formatted (funcall 'recursive_routing_trace    container  (rest lis )  (+  indent  "  ") )))))#|line 537|#
        ))                                                                                                              #|line 538|#
)
                      ...
                      (loop for child_desc in (cdr (assoc 'children  desc))
                        do                                                                                              #|line 551|#
                            (let ((child_instance (funcall 'get_component_instance    reg  (cdr (assoc 'name  child_desc))   container #|line 552|#)))
                                (cdr (assoc '(append child_instance                                                     #|line 553|#)  children))
...
(defun fifo_is_empty (&optional  fifo)                                                                                  #|line 617|#
    (return-from fifo_is_empty (cdr (assoc '(empty )  fifo)))                                                           #|line 618|##|line 619|#
...
(defun force_tick (&optional  parent  eh)                                                                               #|line 665|#
    (let ((tick_msg (funcall 'make_message    "."  (new_datum_tick )                                                    #|line 666|#)))
...
(defun push_message (&optional  parent  receiver  inq  m)                                                               #|line 671|#
    (cdr (assoc '(put m                                                                                                 #|line 672|#)  inq))
      (cdr (assoc '(cdr (assoc '(put receiver                                                                           #|line 673|#)  visit_ordering))  parent))#|line 674|#
)
...
(defun step_child (&optional  child  msg)                                                                               #|line 681|#
    (let ((before_state (cdr (assoc ' state  child))))                                                                  #|line 682|#
        (cdr (assoc '(handler child   msg                                                                               #|line 683|#)  child))
          (let ((after_state (cdr (assoc ' state  child))))                                                             #|line 684|#
              (return-from step_child (values ( and  ( equal    before_state  "idle") (not (equal   after_state  "idle"))) #|line 685|#( and  (not (equal   before_state  "idle")) (not (equal   after_state  "idle"))) #|line 686|#( and  (not (equal   before_state  "idle")) ( equal    after_state  "idle"))))#|line 687|#))#|line 688|#
)
(defun save_message (&optional  eh  msg)                                                                                #|line 690|#
    (cdr (assoc '(cdr (assoc '(put msg                                                                                  #|line 691|#)  saved_messages))  eh))#|line 692|#
)
(defun fetch_saved_message_and_clear (&optional  eh)                                                                    #|line 694|#
    (return-from fetch_saved_message_and_clear (cdr (assoc '(cdr (assoc '(get )  saved_messages))  eh)))                #|line 695|##|line 696|#
)
                      ...
                      (cond
                        ((not (cdr (assoc '(cdr (assoc '(empty )  inq))  child)))                                       #|line 703|#
                              (let ((msg (cdr (assoc '(cdr (assoc '(get )  inq))  child))))                             #|line 704|#
                                  (multiple-value-setq ( began_long_run  continued_long_run  ended_long_run) (step_child child   msg #|line 705|#))
                                    (cond
                                      ...
                                      ( ended_long_run                                                                  #|line 710|#
                                            (funcall 'log_inout  :container  container :component  child :in_message (fetch_saved_message_and_clear child ) )#|line 711|#
                                        )
                                      (t                                                                                #|line 712|#
                              ...
                              (cond
                                ((not (equal  (cdr (assoc ' state  child))  "idle"))                                    #|line 716|#
                                      (let ((msg (force_tick container   child                                          #|line 717|#)))
                                          (cdr (assoc '(handler child   msg                                             #|line 718|#)  child))
...
(defun is_tick (&optional  msg)                                                                                         #|line 739|#
    (return-from is_tick ( equal    "tick" (cdr (assoc '(cdr (assoc '(kind )  datum))  msg))))                          #|line 740|##|line 741|#
...
(defun child_is_ready (&optional  eh)                                                                                   #|line 785|#
    (return-from child_is_ready ( or  ( or  ( or  (not (cdr (assoc '(cdr (assoc '(empty )  outq))  eh))) (not (cdr (assoc '(cdr (assoc '(empty )  inq))  eh)))) (not (equal  (cdr (assoc ' state  eh))  "idle"))) (any_child_ready eh )))#|line 786|##|line 787|#
)
(defun print_routing_trace (&optional  eh)                                                                              #|line 789|#
    (funcall 'print   (routing_trace_all eh )                                                                           #|line 790|#)#|line 791|#
)
(defun append_routing_descriptor (&optional  container  desc)                                                           #|line 793|#
    (cdr (assoc '(cdr (assoc '(put desc                                                                                 #|line 794|#)  routings))  container))#|line 795|#
)
(defun log_connection (&optional  container  connector  message)                                                        #|line 797|#
    (cond
      ...
      (t                                                                                                                #|line 815|#
            (funcall 'print    (concatenate 'string  "*** FATAL error: in log_connection /"  (concatenate 'string (cdr (assoc ' direction  connector))  (concatenate 'string  "/ /"  (concatenate 'string (cdr (assoc ' port  message))  (concatenate 'string  "/ /"  (concatenate 'string (cdr (assoc '(cdr (assoc '(srepr )  datum))  message))  "/")))))) #|line 816|#)
              (exit )                                                                                                   #|line 817|#
        ))                                                                                                              #|line 818|#
)
(defun container_injector (&optional  container  message)                                                               #|line 820|#
    (funcall 'log_inject  :receiver  container :port (cdr (assoc ' port  message)) :msg  message                        #|line 821|#)
      (container_handler container   message                                                                            #|line 822|#)#|line 823|#
)
                            ...
                            (let ((owner_name  ""))                                                                     #|line 55|#
                                (let ((instance_name  template_name))                                                   #|line 56|#
                                    (cond
                                      ...
                                      (let ((instance (cdr (assoc '(funcall 'instantiator    reg   owner   instance_name  (cdr (assoc ' template_data  template)) #|line 62|#)  template))))
...
(defun print_output_list (&optional  eh)                                                                                #|line 239|#
    (loop for m in (funcall 'list   (cdr (assoc '(cdr (assoc ' queue  outq))  eh)) )
      do                                                                                                                #|line 240|#
          (funcall 'print   (format_message m ) )                                                                       #|line 241|#
      )                                                                                                                 #|line 242|#
)
            ...
            (cond
              ((not (equal   datum  nil))                                                                               #|line 271|#
                    (cond
                      ...
                      (funcall 'print   (cdr (assoc '(srepr )  datum)) :file  outf )                                    #|line 276|#
                ))))                                                                                                    #|line 277|#
)
...
(defun deracer_instantiate (&optional  reg  owner  name  template_data)                                                 #|line 356|#
    (let ((name_with_id (gensymbol "deracer"                                                                            #|line 357|#)))
        (let ((inst (funcall 'Deracer_Instance_Data    "idle"  (TwoMessages nil   nil )                                 #|line 358|#)))
                    ...
                    (let (( f (funcall 'open   (cdr (assoc ' filename  inst))   "w"                                     #|line 439|#)))
                        (cond
                          ((not (equal   f  nil))                                                                       #|line 440|#
                                (cdr (assoc '(funcall 'write   (cdr (assoc '(cdr (assoc '(srepr )  datum))  msg))       #|line 441|#)  f))
                                  (cdr (assoc '(close )  f))                                                            #|line 442|#
                                    (funcall 'send    eh   "done"  (new_datum_bang )   msg )                            #|line 443|#
                            )
                          (t                                                                                            #|line 444|#
    ...
    (let (( inst (cdr (assoc ' instance_data  eh))))                                                                    #|line 461|#
        (cond
          (( equal    "1" (cdr (assoc ' port  msg)))                                                                    #|line 462|#
                (setf (cdr (assoc ' buffer1  inst)) (funcall 'clone_string   (cdr (assoc '(cdr (assoc '(srepr )  datum))  msg)) #|line 463|#))
          ...
          (( equal    "2" (cdr (assoc ' port  msg)))                                                                    #|line 466|#
                (setf (cdr (assoc ' buffer2  inst)) (funcall 'clone_string   (cdr (assoc '(cdr (assoc '(srepr )  datum))  msg)) #|line 467|#))
...
(defun print_error_maybe (&optional  main_container)                                                                    #|line 556|#
    (let ((error_port  "✗"))                                                                                            #|line 557|#
        (let ((err (fetch_first_output main_container   error_port                                                      #|line 558|#)))
            (cond
              (( and  (not (equal   err  nil)) ( <   0 (funcall 'len   (funcall 'trimws   (cdr (assoc '(srepr )  err)) ) )))#|line 559|#
...
(defun dump_hierarchy (&optional  main_container)                                                                       #|line 578|#
    (print )                                                                                                            #|line 579|#
      (funcall 'print    (concatenate 'string  "___ Hierarchy ___" (build_hierarchy main_container ))                   #|line 580|#)#|line 581|#
)
...
(defun initialize_stock_components (&optional  reg)                                                                     #|line 646|#
    (funcall 'register_component    reg  (Template "1then2"   nil   deracer_instantiate )                               #|line 647|#)
      (funcall 'register_component    reg  (Template "?"   nil   probe_instantiate )                                    #|line 648|#)
        (funcall 'register_component    reg  (Template "?A"   nil   probeA_instantiate )                                #|line 649|#)
          (funcall 'register_component    reg  (Template "?B"   nil   probeB_instantiate )                              #|line 650|#)
            (funcall 'register_component    reg  (Template "?C"   nil   probeC_instantiate )                            #|line 651|#)
              (funcall 'register_component    reg  (Template "trash"   nil   trash_instantiate )                        #|line 652|#)#|line 653|#
                (funcall 'register_component    reg  (Template "Low Level Read Text File"   nil   low_level_read_text_file_instantiate ) #|line 654|#)
                  (funcall 'register_component    reg  (Template "Ensure String Datum"   nil   ensure_string_datum_instantiate ) #|line 655|#)#|line 656|#
                    (funcall 'register_component    reg  (Template "syncfilewrite"   nil   syncfilewrite_instantiate )  #|line 657|#)
                      (funcall 'register_component    reg  (Template "stringconcat"   nil   stringconcat_instantiate )  #|line 658|#)
                        #|  for fakepipe |#                                                                             #|line 659|#
                          (funcall 'register_component    reg  (Template "fakepipename"   nil   fakepipename_instantiate ) #|line 660|#)#|line 661|#
)                                                                                                                       #|line 663|#
                            ...
                            (let (( main_container (get_component_instance palette   main_container_name :owner  nil    #|line 682|#)))
                                (cond
                                  (( equal    nil  main_container)                                                      #|line 683|#
                                        (funcall 'load_error    (concatenate 'string  "Couldn't find container with page name "  (concatenate 'string  main_container_name  (concatenate 'string  " in files "  (concatenate 'string  diagram_names  "(check tab names, or disable compression?)")))) #|line 687|#)#|line 688|#
                                    ))
                                  (cond
                                                                    ...
                                                                    (cond
                                                                      ( show_traces                                     #|line 704|#
                                                                            (print "--- routing traces ---"             #|line 705|#)
                                                                              (funcall 'print   (routing_trace_all main_container ) #|line 706|#)#|line 707|#
                                                                        ))                                              #|line 708|#
                                                            ))
                                                          (cond
