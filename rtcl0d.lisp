                  ...
                  (let (source_component  nil)
                    (let (target_component  nil)
                      (let (connector (Connector ))
                        (cond 
                          (( ==  (gethash 'dir proto_conn) enumDown) 
                            (progn
                          ...
                          (( ==  (gethash 'dir proto_conn) enumAcross) 
                            (progn
                                      ...
                                      (progn
                                        (let ((slot-value 'sender connector) (Sender  (slot-value 'name source_component)  source_component  (gethash 'source_port proto_conn)))
                                          (let ((slot-value 'receiver connector) (Receiver  (slot-value 'name me)  (slot-value 'outq container)  (gethash 'target_port proto_conn)  me))
                                            (slot-value '(append  connector) connectors))))))))))
                          (( ==  (gethash 'dir proto_conn) enumThrough) 
                            (progn
...
(defclass Sender:
  ...
  (defun __init__  name component port
...
(defclass Receiver:
  ...
  (defun __init__  name queue port component
...
(defun sender_eq (s1 s2)
  (progn
    (let (same_components  ( ==  (slot-value 'component s1) (slot-value 'component s2)))
      (let (same_ports  ( ==  (slot-value 'port s1) (slot-value 'port s2)))
...
(defun is_self (child container)
  (progn
    ...
    (return-from is_self ( ==  child container))))
  ...
  (progn
    (let (before_state (slot-value 'state child))
      ...
      (let (after_state (slot-value 'state child))
        ...
        (return-from step_child (values ( !=  ( and  ( ==  before_state "idle") after_state) "idle")  ( !=  ( and  ( !=  before_state "idle") after_state) "idle") ( ==  ( and  ( !=  before_state "idle") after_state) "idle")))))))
...
(defun attempt_tick (parent eh)
  (progn
    (cond 
      (( !=  (slot-value 'state eh) "idle") 
        (progn
          (force_tick  parent  eh))))))
(defun is_tick (msg)
  (progn
    
    (return-from is_tick ( ==  "tick" (slot-value '(slot-value '(kind ) datum) msg)))))
...
(defun child_is_ready (eh)
  (progn
    ...
    (return-from child_is_ready ( or  ( or  ( or   (not  (slot-value '(slot-value '(empty ) outq) eh))  (not  (slot-value '(slot-value '(empty ) inq) eh)))  ( !=  (slot-value 'state eh) "idle"))  (any_child_ready  eh)))))
...
(defun log_connection (container connector message)
  (progn
    (cond 
      (( ==  "down" (slot-value 'direction connector)) 
        (progn
      ...
      (( ==  "up" (slot-value 'direction connector)) 
        (progn
      ...
      (( ==  "across" (slot-value 'direction connector)) 
        (progn
      ...
      (( ==  "through" (slot-value 'direction connector)) 
        (progn
...
(defclass Component_Registry:
  ...
  (defun __init__ 
    ...
    self.templates = (make-hash-table :test :equal)))
(defclass Template:
  
  (defun __init__  name="" template_data= nil instantiator= nil
  ...
  (progn
    ...
    (defun first_char_is (s c)
      (progn
        ...
        (return-from first_char_is ( ==  c (first_char  s)))))
    (defun run_command (eh cmd s)
      (progn
        (let (ret (slot-value '(run  cmd   t  s  'utf_8') subprocess))
          (cond 
            ((not  ( ==  (slot-value 'returncode ret) 0)) 
              (progn
                (cond 
                  (( !=  (slot-value 'stderr ret)  nil) 
                    (progn
    ...
    (defclass Eh:
      ...
      (defun __init__ 
        ...
        self.name = ""
        self.inq = (slot-value '(Queue ) queue)
        self.outq = (slot-value '(Queue ) queue)
        self.owner =  nil
        self.saved_messages = (slot-value '(LifoQueue ) queue)
        self.inject = injector_NIY
        self.children =  nil
        self.visit_ordering = (slot-value '(Queue ) queue)
        self.connections =  nil
        self.routings = (slot-value '(Queue ) queue)
        self.handler =  nil
        self.instance_data =  nil
        self.state = "idle"
        self.kind =  nil
        self.trace =  nil
        self.depth = 0))
    ...
    (defclass TwoMessages:
      ...
      (defun __init__  first= nil second= nil
        
        self.first = first
        self.second = second))
    (defclass Deracer_Instance_Data:
      
      (defun __init__  state="idle" buffer= nil
    ...
    (defun deracer_instantiate (reg owner name template_data)
      (progn
        (let (name_with_id (gensym  "deracer"))
          (let (inst (Deracer_Instance_Data  (TwoMessages )))
      ...
      (progn
        (let (inst (slot-value 'instance_data eh))
          (cond 
            ...
            (( ==  (slot-value 'state inst) "waitingForFirst") 
              (progn
                (cond 
                  (( ==  "1" (slot-value 'port msg)) 
                    (progn
            ...
            (( ==  (slot-value 'state inst) "waitingForSecond") 
              (progn
                (cond 
                  (( ==  "2" (slot-value 'port msg)) 
                    (progn
    ...
    (defun ensure_string_datum_handler (eh msg)
      (progn
        (cond 
          (( ==  "string" (slot-value '(slot-value '(kind ) datum) msg)) 
            (progn
              (forward  eh  ""  msg)))
          (t 
            (progn
    ...
    (defun maybe_stringconcat (eh inst msg)
      (progn
        (cond 
          (( and   ( ==  0 (len  (slot-value 'buffer1 inst)))  ( ==  0 (len  (slot-value 'buffer2 inst)))) 
            (progn
          ...
          (( >=  (slot-value 'count inst) 2) 
            (progn
              (let (concatenated_string "")
                (cond 
                  (( ==  0 (len  (slot-value 'buffer1 inst))) 
                    (progn
                      (let (concatenated_string (slot-value 'buffer2 inst)))))
                  (( ==  0 (len  (slot-value 'buffer2 inst))) 
                    (progn
      ...
      (progn
        (let (error_port "✗")
          (let (err (fetch_first_output  main_container  error_port))
            (cond 
              (( and   ( !=  err  nil)  ( <  0 (len  (trimws  (slot-value '(srepr ) err))))) 
                (progn
    ...
    (defclass OhmJS_Instance_Data:
      ...
      (defun __init__ 
        ...
        self.pathname_0D_ =  nil
        self.grammar_name =  nil
        self.grammar_filename =  nil
        self.semantics_filename =  nil
        self.s =  nil))
    ...
    (defun ohmjs_maybe (eh inst causingMsg)
      (progn
        (cond 
          (( !=  ( and  ( !=  ( and  ( !=  ( and  ( !=  ( and  ( !=   nil (slot-value 'pathname_0D_ inst))  nil) (slot-value 'grammar_name inst))  nil) (slot-value 'grammar_filename inst))  nil) (slot-value 'semantics_filename inst))  nil) (slot-value 's inst)) 
            (progn
    ...
    (defun initialize_stock_components (reg)
      (progn
        (register_component  reg  (Template  "1then2"  deracer_instantiate))
        (register_component  reg  (Template  "?"  probe_instantiate))
        (register_component  reg  (Template  "?A"  probeA_instantiate))
        (register_component  reg  (Template  "?B"  probeB_instantiate))
        (register_component  reg  (Template  "?C"  probeC_instantiate))
        (register_component  reg  (Template  "trash"  trash_instantiate))
        (register_component  reg  (Template  "Low Level Read Text File"  low_level_read_text_file_instantiate))
        (register_component  reg  (Template  "Ensure String Datum"  ensure_string_datum_instantiate))
        (register_component  reg  (Template  "syncfilewrite"  syncfilewrite_instantiate))
        (register_component  reg  (Template  "stringconcat"  stringconcat_instantiate))
        (register_component  reg  (Template  "fakepipename"  fakepipename_instantiate))
        (register_component  reg  (Template  "OhmJS"  ohmjs_instantiate))))
